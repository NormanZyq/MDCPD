...
@Date: 2017-11
@Last Modified by: Yeqi Zhu 2023
"""
import os
import warnings
...
import scipy.stats
from src.baselines.change_point.cicpd_components.cicpd_evaluation import Evaluation
...
toynetwork0406_Ground = {1, 3, 4, 6, 7, 9, 10, 12}
MIT_Ground_Week = {1, 7, 8, 10, 11, 12, 14, 15, 16, 18, 20, 22, 26, 27, 29, 30, 36, 37, 38, 39}

Enron_Ground_month = {12, 14, 20, 21, 23, 24, 25, 25, 26, 28, 29, 30, 32, 32, 33, 36, 36, 36, 36, 37, 38, 39, 39, 39,
                      40}
Cosponsor_Ground = {3, 7}
rdyn_case1_ground = set(
    [n + 1 for n in {5, 10, 13, 17, 29, 32, 35, 41, 46, 50, 53, 57, 60, 62, 65, 69, 73, 77, 82, 86, 91, 94, 97, 100}])
...
def constructNetwork(tempM, flag):
    ...
    """
    if flag:
        graph = nx.from_numpy_array(tempM)
    ...
    graph = nx.from_numpy_array(newtempM)
    graph = graph.to_directed()  #
    return graph
...
def dealsome(pr, flag):
    ...
    sortedResult = OrderedDict(sorted(pr.items(), key=itemgetter(1), reverse=True))
    top = [0] * len(pr)
...
def detect_change_points(path, groundEvent, nodeImportance, alpha_pg, flag1, flag2,
    ...
    for alpha in range(1, 101, 1):
        ...
        condensed_similarity = list(map(func, distance_array))  # modified by zyq
        ...
        if not _graph_is_connected(adjacentM):
            ...
            graph_weighted = nx.from_numpy_array(adjacentM)
    ...
    return p_c, temp_f, temp_k, temp_alpha
