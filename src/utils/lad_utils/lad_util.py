...
def plot_ranks(anoRank, proRank, fname):
    ...
    ax.plot(t, proRank, marker="P", color=colors[1], ls='solid', linewidth=0.5, markersize=1,
            label="Graph property Rank")
...
def plot_edges(G_times, fname):
    ...
    fig = plt.figure(figsize=(4, 2))
    ax = fig.add_subplot(1, 1, 1)
    ax.plot(t, cumulative_edges, marker='o', color='#74a9cf', ls='solid', linewidth=0.5, markersize=1,
...
def all_plots_in_one(G_times, fname):
    ...
    for ax in axs:
        ...
        if (plot_num == 0):
            ax.plot(t, max_weight, marker="P", color='#ffa600', ls='solid', linewidth=0.5, markersize=1,
                    label="maximum weight")
            ax.plot(t, min_weight, marker="h", color='#003f5c', ls='solid', linewidth=0.5, markersize=1,
                    label="minimum weight")
            ax.plot(t, avg_weight, marker="o", color='#bc5090', ls='solid', linewidth=0.5, markersize=1,
        ...
        if (plot_num == 2):
            ax.plot(t, num_strong, marker="P", color='#ffa600', ls='solid', linewidth=0.5, markersize=1,
                    label="strongly")
            ax.plot(t, num_weak, marker="h", color='#003f5c', ls='solid', linewidth=0.5, markersize=1, label="weakly")
            outliers = find_rarity_windowed_outlier(num_weak)  # use weakly
            outliers.sort()
...
def all_in_one_compare(G_times, fname, label_sets, directed, window, initial_window, percent_ranked):
    ...
    for ax in axs:
        ...
        if (plot_num == 0):
            ...
            outliers = find_rarity_windowed_outlier(avg_weight, percent_ranked=percent_ranked, window=window,
                                                    initial_period=initial_window)  # use weakly
            outliers.sort()
            colors = ['r', 'b', 'g', 'c', 'm', 'y']
            c_idx = 0
            # for xc in outliers:
        ...
        if (plot_num == 1):
            ...
            outliers = find_rarity_windowed_outlier(num_nodes, percent_ranked=percent_ranked, window=window,
                                                    initial_period=initial_window)
            outliers.sort()
            colors = ['r', 'b', 'g', 'c', 'm', 'y']
            c_idx = 0
            # for xc in outliers:
        ...
        if (plot_num == 2):
            ...
            outliers = find_rarity_windowed_outlier(num_edges, percent_ranked=percent_ranked, window=window,
                                                    initial_period=initial_window)
            outliers.sort()
            colors = ['r', 'b', 'g', 'c', 'm', 'y']
            c_idx = 0
        ...
        if (plot_num == 3):
            if (directed):
                ax.plot(t, num_strong, marker="P", color='#ffa600', ls='solid', linewidth=0.5, markersize=1,
                        label="strongly")
                ax.plot(t, num_weak, marker="h", color='#003f5c', ls='solid', linewidth=0.5, markersize=1,
                        label="weakly")
            else:
                ax.plot(t, num_connected_components, marker="P", color='#ffa600', ls='solid', linewidth=0.5,
                        markersize=1)

            if (directed):
                outliers = find_rarity_windowed_outlier(num_weak, percent_ranked=percent_ranked, window=window,
                                                        initial_period=initial_window)  # use weakly
            else:
                outliers = find_rarity_windowed_outlier(num_connected_components, percent_ranked=percent_ranked,
                                                        window=window, initial_period=initial_window)
            outliers.sort()
            colors = ['r', 'b', 'g', 'c', 'm', 'y']
            c_idx = 0
